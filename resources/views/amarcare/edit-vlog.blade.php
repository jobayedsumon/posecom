@extends('layouts.app', ['activePage' => 'edit-vlog', 'titlePage' => __('Edit Vlog')])
...
@section('content')
    <div class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <form method="post" action="{{ route('amar-care.update', $amarcare->id) }}" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                        <div class="card ">
                            <div class="card-header card-header-danger">
                                <h4 class="card-title">{{ __('Edit Vlog') }}</h4>
                            ...
                            <div class="card-body ">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="title" id="input-name" type="text" placeholder="{{ __('Vlog Title') }}"
                                                   value="{{ $amarcare->title }}" required="true" aria-required="true"/>
                                            @if ($errors->has('name'))
                                                ...
                                                <div class="inline-block relative w-64">
                                                    <select name="category_id" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                                                        @forelse($categories as $category)
                                                            <option {{ $category->id == $amarcare->category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->name }}</option>
                                                            <hr>
                                                        @empty
                                                        @endforelse
                                                    </select>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="{{ $errors->has('image_primary') ? ' has-danger' : '' }}">
                                            <input class="form-control{{ $errors->has('image_primary') ? ' is-invalid' : '' }}"
                                                   name="video" id="input-image_primary" value="{{ $amarcare->video }}" type="text" required />
                                            @if ($errors->has('image_primary'))
                                                <span id="image_primary-error" class="error text-danger" for="input-image_primary">{{ $errors->first('image_primary') }}</span>
                                            @endif
                                        </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-7">
                                        <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                                            <textarea class="w-100" name="description" id="" cols="" rows="10">{{ $amarcare->description }}</textarea>
